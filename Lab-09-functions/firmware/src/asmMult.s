 ...
 *              r1: address where to store unpacked, sign-extended 32 bit a value
 *              r2: address where to store unpacked, sign-extended 32 bit b value
 *    outputs:  r0: No return value
 *              memory: store unpacked A value in location specified by r1
 *                      store unpacked B value in location specified by r2
 */
asmUnpack:   
 ...
 *    outputs:  r0: initial product: 
 *                   product of: abs value of multiplicand (a) * abs value of multiplier (b)
 */    
asmMult:   
...
/* function: asmFixSign
 *    inputs:   r0: initial product: (abs value of A) * (abs value of B)
 *              r1: sign bit of originally unpacked value of A
 *              r2: sign bit of originally unpacked value of B
 *    outputs:  r0: final product: sign-corrected version of initial product
 */    
asmFixSign:   
...
/* function: asmMain
 *    inputs:   r0: contains packed value to be multiplied using shift-and-add algorithm
 *                  MSB 16bits is signed multiplicand (a)
 *                  LSB 16bits is signed multiplier (b)
 *    outputs:  r0: final product: sign-corrected product of the two unpacked A and B input values
 */    
asmMain:   
     ...
     */
     ...
     /* Step 3:
      * call asmMult. Pass a_Abs as the mutliplicand, and b_Abs as the multiplier.
      * asmMult returns the initial (positive) product in r0.
      * In this function (asmMain), store the output to init_Product.
      * Store the value returned in r0 to mem location init_Product.
      */



     /* Step 4:
      * call asmFixSign. Pass in the initial product, and the sign bits for the original 
      * a and b inputs. asmFixSign returns the final product with the correct sign.
